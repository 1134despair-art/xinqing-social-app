...
/**
 * 调用约束：
 *   - 必须在识别已开始（即收到 realTimeRecognizerOnFlowRecognizeStart: 之后）
 *     且尚未结束之前调用，否则返回的 NSError 的 code 为 QCloudRealTimeClientErrCode_NotRunning。
 *
 * @param contextPrompt 上下文提示数据，prompt 至少包含一个有效项；
 *                      为 nil / prompt 为空时返回的 NSError 的 code 为 QCloudRealTimeClientErrCode_InvalidParam；
 *                      序列化失败或底层 WebSocket 发送异常时返回 QCloudRealTimeClientErrCode_WriteContentFailed
 *                      （此时 NSError.userInfo[NSUnderlyingErrorKey] 携带底层原始错误，便于排查）。
 * @return nil 表示已成功投递；非 nil 即失败原因，可通过 error.code 判断属于哪种失败。
 */
- (nullable NSError *)writeContent:(QCloudContextPrompt *)contextPrompt;
